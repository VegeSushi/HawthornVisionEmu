...
public:
    ...
    static const size_t MAX_WORDS = 128; // Safe from colliding with 0xFE/0xFF opcodes
#else
...
private:
    ...
    int16_t sp = -1; // CRITICAL FIX: Upgraded to int16_t so 256 stack sizes don't overflow math logic
    ...
    void execute_word(uint8_t idx) {
        ...
        if (words[idx].type == BUILTIN) {
            ...
            while (ptr < DICT_SIZE && dict[ptr] != 0xFF && last_error == OK) {
                // CRITICAL FIX: The Literal Execution Engine
                if (dict[ptr] == 0xFE) {
                    ptr++; // Move past literal marker
                    int32_t val;
                    memcpy(&val, &dict[ptr], sizeof(int32_t)); // Extract the 4 byte integer
                    push(val);
                    ptr += sizeof(int32_t);
                } else {
                    execute_word(dict[ptr]);
                    ptr++;
                }
            }
        }
...
public:
    ...
    Status interpret(const char* input) {
        ...
        while (idx < len && last_error == OK) {
            ...
            if (compile_mode) {
                if (strcmp(token, ";") == 0) {
                    ...
                    if (found >= 0 && dp < DICT_SIZE) {
                        ...
                        // CRITICAL FIX: The Literal Compiler Engine
                        ...
                        if (is_numeric && dp + 5 < DICT_SIZE) {
                            dict[dp++] = 0xFE; // Insert Literal byte marker
                            int32_t val = (int32_t)strtol(token, nullptr, 10);
                            memcpy(&dict[dp], &val, sizeof(int32_t)); // Compile number natively
                            dp += sizeof(int32_t);
                        } else {
                            last_error = COMPILE_ERROR;
                        }
            ...
            int8_t found_idx = -1;
            for (int8_t i = word_count - 1; i >= 0; i--) {
                if (strcmp(words[i].name, token) == 0) {
                    found_idx = i;
                    break;
                }
            }
